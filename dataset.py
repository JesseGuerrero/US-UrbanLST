...
class LandsatSequenceDataset(Dataset):
    def __init__(
        ...
        max_output_nodata_pct: float = 0.0,
        ...
        self.max_output_nodata_pct = max_output_nodata_pct
        self.cities_filter = cities
    ...
    def _get_cache_filename(self) -> str:
        ...
        config_str += f"_{self.debug_monthly_split}_{self.debug_year}_{self.max_input_nodata_pct}_{self.max_output_nodata_pct}"
        
        # Add cities filter to hash
        if self.cities_filter is not None:
            cities_str = '_'.join(sorted(self.cities_filter))
            config_str += f"_{cities_str}"
        ...
        if len(self.remove_channels) > 0:
            for removed_channel in self.remove_channels:
                suffix += f"_r{removed_channel}"
        if self.cities_filter is not None:
            suffix += f"_c{'_'.join(sorted(self.cities_filter))}"
        
        cache_filename = str(cache_dir / f"sequences_{self.split}_{self.cluster}_{config_hash}{suffix}.pkl")
        ...
        for i in range(len(sorted_months) - self.total_sequence_length + 1):
            ...
            if self._are_consecutive_months(input_months + output_months):
                if self._verify_tile_sequence_exists(city, tile_row, tile_col, input_months + output_months):
                    ...
                    # Check if any output scenes exceed max NODATA threshold
                    has_excessive_output_nodata = False
                    for month in output_months:
                        output_nodata_pct = self._get_nodata_pct_in_lst_tile(city, month, tile_row, tile_col, monthly_scenes)
                        if output_nodata_pct > self.max_output_nodata_pct:
                            has_excessive_output_nodata = True
                            break
                    
                    # Check if input sequence has excessive NODATA
                    has_excessive_input_nodata = False
                    if not has_excessive_output_nodata:
                    ...
                    if not has_excessive_output_nodata and not has_excessive_input_nodata:
    ...
    def _generate_interpolated_sequences(self, city: str, tile_row: int, tile_col: int, 
        ...
        while current_start <= max_date:
            ...
            if has_excessive_output_nodata:
                current_start += relativedelta(months=1)
                continue
    ...
    def _get_nodata_pct_in_lst_tile(self, city: str, month: str, tile_row: int, tile_col: int, monthly_scenes: Dict[str, str]) -> float:
        """Get NODATA percentage in LST tile (value 0)"""
        try:
            scene_dir = Path(monthly_scenes[month])
            lst_tile_path = scene_dir / f"LST_row_{tile_row:03d}_col_{tile_col:03d}.tif"
            
            if not lst_tile_path.exists():
                return 1.0  # Missing file counts as 100% NODATA
            ...
            with rasterio.open(lst_tile_path) as src:
                ...
                # Count NODATA pixels (value 0 or actual nodata value)
                nodata_value = src.nodata if src.nodata is not None else 0
                nodata_count = np.sum((lst_data == 0) | (lst_data == nodata_value))
    ...
    def _get_all_cities(self) -> List[str]:
        ...
        # Filter to specific cities if provided
        if self.cities_filter is not None:
            all_cities = [c for c in all_cities if c in self.cities_filter]
            print(f'Filtered to {len(all_cities)} specified cities: {all_cities}')
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            ...
            self.train_dataset = LandsatSequenceDataset(
                ...
                max_output_nodata_pct=self.max_output_nodata_pct,
                remove_channels=self.remove_channels,
                cities=self.cities_filter
            )
