...
def get_cache_filename(
    ...
    config_str += f"_{debug_monthly_split}_{debug_year}_{max_input_nodata_pct}_{max_output_nodata_pct}"
    
    # Add cities filter to hash
    if cities is not None:
        cities_str = '_'.join(sorted(cities))
    ...
    if cities is not None:
        suffix += f"_c{'_'.join(sorted(cities))}"
    
    return str(cache_dir / f"sequences_{split}_{cluster}_{config_hash}{suffix}.pkl")
...
def build_split_cache(
    ...
    cache_file = get_cache_filename(
        ...
        max_output_nodata_pct=max_output_nodata_pct,
    ...
    if os.path.exists(cache_file) and not force_rebuild:
        try:
            ...
            print(f"⚠ Failed to load existing cache ({e}), rebuilding...")
...
def main():
    ...
    parser.add_argument("--cities", type=str, nargs="*",
                        default=None,
                        help="Filter to specific cities (e.g., 'San Antonio_TX')")
    
    # Quality filtering
    parser.add_argument("--max_nodata", type=float, default=0.60,
                        help="Maximum input NODATA percentage (0.0-1.0)")
    parser.add_argument("--max_output_nodata", type=float, default=0.0,
                        help="Maximum output/target NODATA percentage (0.0-1.0)")
    ...
    if not 0.0 <= args.max_output_nodata <= 1.0:
        parser.error("--max_output_nodata must be between 0.0 and 1.0")
    ...
    if args.check_only:
        ...
        for split in args.splits:
            cache_file = get_cache_filename(
                ...
                max_output_nodata_pct=args.max_output_nodata,
